...
import '../../../utilities/functions/webview_func.dart';
...
class LocationProvider extends ChangeNotifier{
  GeocodingModel _geocodingModel = GeocodingModel(latitude: 0.0, longitude: 0.0, address: '');
  ...
  Future<GeocodingModel> getCurrentUserLocation(BuildContext context, {BuildContext? dialogContext, bool? initFromMaps}) async {
    try {
      bool conn = await checkInternetConnectivity();
      if (!conn) return (initFromMaps != null) ? GeocodingModel(latitude: -6.1754083, longitude: 106.8245787, address: 'Monas, Jakarta') : GeocodingModel(latitude: 0.0, longitude: 0.0, address: '');
      bool resp = await PermissionSettingProvider.read(context).requestLocationPermission(context: dialogContext);
      if (!resp) return GeocodingModel(latitude: 0.0, longitude: 0.0, address: '');
      ...
      return GeocodingModel(latitude: 0.0, longitude: 0.0, address: '');
    }
  }
  ...
  Future<GeocodingModel> getUserLatLong({required String kelurahan, required String kecamatan, required String kota, required String provinsi}) async {
    try {
      ...
      bool conn = await checkInternetConnectivity();
      if (!conn) return GeocodingModel(latitude: 0.0, longitude: 0.0, address: '');
      List<Location> locations = await locationFromAddress(address);
      if (locations.isEmpty) return GeocodingModel(latitude: 0.0, longitude: 0.0, address: address);
  ...
  void setLatLong(double latitude, double longitude) {
    clog('Set New Latitude and Longitude');
    _geocodingModel = GeocodingModel(latitude: latitude, longitude: longitude, address: '');
  }

  Future<void> getRouteOnGoogleMaps(BuildContext context) async {
    if (geocodingModel.latitude != 0.0 || geocodingModel.longitude != 0.0) return await launchURL('https://www.google.com/maps/dir//${geocodingModel.latitude},${geocodingModel.longitude}');
    return showToastTop(context, 'Koordinat belum sepenuhnya ditetapkan! Harap tunggu sebentar');
  }

  /// Fungsi untuk mereset semua value
  void reset(){
    _geocodingModel = GeocodingModel(latitude: 0.0, longitude: 0.0, address: '');
    _thoroughfare = '';
    _subThoroughfare = '';
    _subLocality = '';
    _locality = '';
  ...
  static LocationProvider read(BuildContext context) => context.read();
  static LocationProvider watch(BuildContext context) => context.watch();
}