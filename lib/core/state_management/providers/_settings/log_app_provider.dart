...
class LogAppSettingProvider extends ChangeNotifier{
  ...
  Future<void> initialize() async {
    try{
      notifyListeners();
      _listLogApp = await SqfliteLogAppServices.getLogApp();
      _listLogAppError = await SqfliteLogAppServices.getLogApp(filterByAppLog: true);
      _listLogResponseError = await SqfliteLogAppServices.getLogApp(filterByApiLog: true);
    } catch (e, s) {
  ...
  Future<void> deleteAllLog() async {
    try{
      bool resp = await SqfliteLogAppServices.deleteAllLogApp();
      if (resp) {
        _listLogApp = [];
        _listLogAppError = [];
  ...
  Future<void> deleteSelectedLog(int id) async {
    try{
      bool resp = await SqfliteLogAppServices.deleteSelectedLogApp(id);
      if (resp){
    ...
    try{
      ...
      if (dateStamp != null){
        ...
        /// Jika tanggal tersimpan kurang dari hari ini, jalankan pengecekan batas ukuran log app
          ...
          await SqfliteLogAppServices.automaticallyDeleteLogAppWhenSizeReached();
        }
