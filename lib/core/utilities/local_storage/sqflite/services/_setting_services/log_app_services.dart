...
import 'package:agent/core/utilities/extensions/primitive_data/int_ext.dart';
...
class SqfliteLogAppServices {
  /// Fungsi untuk menyimpan log ke dalam database
    ...
    try {
      if (!kIsWeb) {
        ...
        await db.insert(
          ...
          LogAppCollection(logDate: DateTime.now(), level: level, statusCode: statusCode, title: title, subtitle: subtitle, description: description, logs: logs).toMap(),
  ...
  /// Fungsi untuk mengambil semua log dari database
  static Future<List<LogAppCollection>> getLogApp({bool? filterByAppLog, bool? filterByApiLog}) async {
    try {
      if (!kIsWeb) {
        ...
        return maps.map((map) => LogAppCollection.fromMap(map)).toList();
      } else {
  ...
  /// Fungsi untuk menghapus semua log dari database
  ...
  /// Fungsi untuk menghapus log spesifik berdasarkan [id] dari database
