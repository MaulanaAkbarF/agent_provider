...
class SuffixIconButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return IconButton(
      icon: Icon(obscureText ? Icons.visibility_off : Icons.visibility, color: ThemeColors.surface(context)),
      onPressed: onPressed,
    );
  }
}
...
class AnimatePasswordField extends StatefulWidget {
  ...
  @override
  AnimatePasswordFieldState createState() => AnimatePasswordFieldState();
}

class AnimatePasswordFieldState extends State<AnimatePasswordField> with SingleTickerProviderStateMixin {
            ...
            child: TextFormField(
              ...
              validator: widget.isRequired == false ? null : (value){
                ...
                if (widget.minInput != null && value.trim().length < widget.minInput!){
                  if (widget.keyboardType == TextInputType.number) {
                    return '⚠️  Harap masukkan lebih dari ${widget.minInput} angka';
                  } else {
                    return '⚠️  Harap masukkan lebih dari ${widget.minInput} karakter';
                  }
                }
                return null;
              },
